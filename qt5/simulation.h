...
using Solver  = Eigen::SparseLU<Eigen::SparseMatrix<double>>;  //Turns out that SparseLU is actually fast enough :)
...
public:
    ...
    Simulation(ostream &_output, Environment* _env, Electrode* _el, Experiment* _exper, System* _sys)
      : output_stream( _output ), env( _env ), el( _el ), exper( _exper ), sys( _sys ) {}
    ~Simulation() {}
